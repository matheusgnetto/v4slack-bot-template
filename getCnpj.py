...
def get_cnpj(cleaned_cnpj, formatted_cnpj):
    ...
    query_cnpj = "SELECT current_phase FROM v4company.PipedeProjetos WHERE cnpj = ? OR cnpj = ?"
    
    cursor.execute(query_cnpj, (cleaned_cnpj, formatted_cnpj))
    current_phase = cursor.fetchone()
    connection.close()
    
    if current_phase:
        if current_phase[0] == 'Concluído':
            return "O CNPJ do Lead consultado é um projeto inativo atualmente!"
        elif current_phase[0] != 'Concluído' and current_phase[0] != 'Arquivado':
            return "O CNPJ do Lead consultado está ativo na base da V4!"
    else:
        return "O CNPJ do Lead consultado não foi encontrado na base de dados da V4!"

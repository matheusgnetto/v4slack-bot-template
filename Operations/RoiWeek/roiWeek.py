...
def update_message_roiweek(event, ack, logger, body, client):
    ack()
    event = body["message"]
...
def update_message_roiweek_yes(event, ack, body, logger, client):
    ack()
    event = body["message"]
    channel_id = body["channel"]["id"]
    thread_ts = event.get("ts", None)
...
def update_message_roiweek_support(event, ack, body, logger, client):
    ack()
    event = body["message"]
    channel_id = body["channel"]["id"]
    thread_ts = event.get("ts", None)
...
def handle_roiweek_support(event, ack, body, logger, client):
    ack()
    ...
    update_message_roiweek_support(event, ack, body, logger, client)

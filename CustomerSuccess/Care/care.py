...
def update_message_care_support(event, ack, body, logger, client):
    ack()
    event = body["message"]
...
def handle_care_support(event, ack, body, logger, client):
    ack()
    ...
    update_message_care_support(event, ack, body, logger, client)
